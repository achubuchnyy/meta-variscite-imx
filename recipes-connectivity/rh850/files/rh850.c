...
struct spi_frame { /* TLV for MISO line */
	u8 len;
	u16 crc;
	u8 data[];
} __packed;
...
static u16 ltc_pec15 (const u8 *data, u8 size) {
    /* precomputed CRC15 Table */
    static const u16 pec15Table [256] =
        {
            0x0000,
            0xc599,
            0xceab,
...
static int rh850_process_rx(struct rh850_can *priv_data, char *rx_buf)
{
	...
	int actual_length = priv_data->xfer_length;
	int ret = 0;
	u16 crc = 0;


	frame = (struct spi_frame *)rx_buf;
	if ((frame->crc & 0x1) != 0) return ret;

	crc = ltc_pec15(&frame->data[0], frame->len);
	if(crc != frame->crc)
	{
		LOGDI("CRC_ERR: calc = %x  rec = %x \n", crc, frame->crc);
...
static int rh850_do_spi_transaction(struct rh850_can *priv_data)
{
	...
	LOGDI("spi_sync ret %d data len=%x crc[0]=%x crc[1]=%x cmd=%x len=%x seq[0]=%x seq[1]=%x %x\n", ret,
			   ...
			   int can_channel, struct can_frame *cf)
{
	...
	*(u16*)(tx_buf+req->len) = ltc_pec15(tx_buf, req->len) ;
	ret = rh850_do_spi_transaction(priv_data);
